 ...
 */
async function fetchGear(shopDomain, accessToken, collectionId) {
   try {
 ...
 */
function selectRandomGear(products) {
 ...
 */
function createGearPromotionMessage(hoodie) {
...
async function promoteRandomGear(client) {
   const shopDomain = process.env.SHOPIFY_STORE_DOMAIN;
   const accessToken = process.env.SHOPIFY_ACCESS_TOKEN;
   const collectionId = process.env.GEAR_COLLECTION_ID;

   const gears = await fetchGear(shopDomain, accessToken, collectionId);
   if (gears.length > 0) {
      const gear = selectRandomGear(gears);
      const embedMessage = createGearPromotionMessage(gear);
...
module.exports = { promoteRandomGear };
